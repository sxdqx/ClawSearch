...
int csMain::FirstScan()
{
	if (m_currentScan > 0) {
		...
		IupSetAttribute(m_hButtonFirstScan, "TITLE", "首次扫描");
	...
	IupSetAttribute(m_hButtonFirstScan, "TITLE", "新扫描");
...
void csMain::SetScanTypeCombo(bool firstScan)
{
	...
	if (firstScan) {
		IupSetAttribute(m_hComboScanType, "1", "精准值");
		IupSetAttribute(m_hComboScanType, "2", "大于...");
		IupSetAttribute(m_hComboScanType, "3", "小于...");
		IupSetInt(m_hComboScanType, "VALUE", (int)m_scanner.m_initialScanType);
		return;
	}

	IupSetAttribute(m_hComboScanType, "1", "精准值");
	IupSetAttribute(m_hComboScanType, "2", "变化的数值");
	IupSetAttribute(m_hComboScanType, "3", "数值不变");
	IupSetAttribute(m_hComboScanType, "4", "大于...");
	IupSetAttribute(m_hComboScanType, "5", "小于...");
	IupSetAttribute(m_hComboScanType, "6", "增加的值");
	IupSetAttribute(m_hComboScanType, "7", "减少的值");
...
void csMain::Open()
{
	...
	m_hButtonFirstScan = IupButton("首次扫描", "ScanFirst");
	m_hButtonNextScan = IupButton("再次扫描", "ScanNext");
	...
	SetScanTypeCombo(true);
	Ihandle* hScanType = IupSetAttributes(IupHbox(IupLabel("扫描类型"), m_hComboScanType, nullptr), "MARGIN=0x0, GAP=5");
	...
	IupSetAttribute(m_hComboValueType, "1", "字节");
	IupSetAttribute(m_hComboValueType, "2", "2 字节");
	IupSetAttribute(m_hComboValueType, "3", "4 字节");
	IupSetAttribute(m_hComboValueType, "4", "8 字节");
	IupSetAttribute(m_hComboValueType, "5", "单浮点");
	IupSetAttribute(m_hComboValueType, "6", "双浮点");
	IupSetInt(m_hComboValueType, "VALUE", (int)m_scanner.m_currentScanValueType);
	Ihandle* hValueType = IupSetAttributes(IupHbox(IupLabel("数值类型"), m_hComboValueType, nullptr), "MARGIN=0x0, GAP=5");
	CLAW_SETCALLBACK(m_hComboValueType, "ACTION", ScanValueTypeChanged);

	m_hCheckFastScan = IupToggle("首次扫描", nullptr);
	...
	m_hCheckPauseWhileScanning = IupToggle("扫描时暂停", nullptr);
	...
	IupSetAttribute(m_hFrameScanOptions, "TITLE", "扫描选项");
